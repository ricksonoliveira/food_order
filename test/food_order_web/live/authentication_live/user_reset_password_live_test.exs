defmodule FoodOrderWeb.UserResetPasswordLiveTest do
    ...
    test "resets password once", %{conn: conn, token: token, user: user} do
      ...
      {:ok, conn} =
        ...
        |> form("#reset_password_form",
          user: %{
            "password" => "New@Val1dPassword",
            "password_confirmation" => "New@Val1dPassword"
          }
        )
        |> render_submit()
      ...
      assert Accounts.get_user_by_email_and_password(user.email, "New@Val1dPassword")
    end
  ...
  describe "Reset password navigation" do
    ...
    test "redirects to password reset page when the Register button is clicked", %{
      ...
      {:ok, conn} =
        lv
        |> element(~s|div a:fl-contains("Register")|)
        |> render_click()
